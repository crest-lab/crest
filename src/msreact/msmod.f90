...
module msmod
      ...
      type msmol
          ...
          integer :: chrg
          integer :: nfrag
          integer,allocatable :: fragi(:) ! index of fragatoms 1 first fragment 2 second fragment
          integer :: fragcount ! number of fragments in product 1: isomer 1< : fragmentation; 0, not determined yet, -1 means we sort this out
          real(wp) :: molmass
          contains
      ...
      !private ! not sure why this was here
contains
...
subroutine readbasicpos(env, nbaseat, basicatlist)
    ...
    write(*, "('Protonation sides at atom positions:', *(i))") (basicatlist(i), i = 1, nbaseat)
    call remove('xtb.out')
...
subroutine fragment_structure(nat,oz,xyz,rcut,at1,at2,frag,fragcount) ! works better than mrec
    use strucrd
    implicit none
    integer  :: at1,at2, nat
    integer  :: i,j
...
! increase distance between fragments, better for transistion state search 
 ! we remove here also multiple fragmented species, and unreasonable fragments( too much distance between fragments)
